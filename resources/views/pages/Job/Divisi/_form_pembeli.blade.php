@can('data-pendukung/pembeli/view')
    <x-job.detail.list-pembeli-component :listPembeli="$jobDivisi->pembeli" />
@endcan

@can('data-pendukung/pembeli/create')
    @if (!count($jobDivisi->pembeli))
        @php
            $oldPembelis =
                old('pembeli') ??
        ...
        <form action="{{ route('job.divisi-data-pendukung.store') }}" method="post" enctype="multipart/form-data" id="form_pembeli">
            @csrf
            <input type="text" hidden name="job_divisi_id" value="{{ $jobDivisi->id }}">

            <div class="body__pembeli">
                @foreach ($oldPembelis as $key => $item)
                    <div class="card shadow-sm border border-light-subtle rounded-3 card_{{ $key }} @if ($key > 0) mt-4 @endif" data-index="{{ $key }}">
                        <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 fw-bold text-dark">
                                <i class="bi bi-people-fill me-2 text-primary"></i>Form Data Pembeli
                            </h6>
                            @if ($key > 0)
                                <button type="button" class="btn btn-sm btn-outline-danger border-0 px-2 py-1" onclick="removePembeli({{ $key }})" title="Hapus Form Ini">
                        ...
                        <div class="card-body pt-3">
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <input type="text"
                                        class="form-control @error("pembeli.$key.nama_lengkap") is-invalid @enderror"
                                        name="pembeli[{{ $key }}][nama_lengkap]"
                                        value="{{ old("pembeli.$key.nama_lengkap", $item['nama_lengkap'] ?? '') }}"
                                        placeholder="Masukkan nama lengkap">
                                    @error("pembeli.$key.nama_lengkap")
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number"
                                        ...
                                        name="pembeli[{{ $key }}][phone]"
                                        value="{{ old("pembeli.$key.phone", $item['phone'] ?? '') }}"
                                        inputmode="tel" placeholder="Contoh: 081234567890">
                                    @error("pembeli.$key.phone")
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="email"
                                        class="form-control @error("pembeli.$key.email") is-invalid @enderror"
                                        name="pembeli[{{ $key }}][email]"
                                        value="{{ old("pembeli.$key.email", $item['email'] ?? '') }}"
                                        autocomplete="email" placeholder="email@domain.com">
                                    @error("pembeli.$key.email")
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
            ...
            @if ($jobDivisi->status !== 'Batal Akad')
                <div class="d-flex justify-content-end mt-4 mb-5">
                    <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow-sm btn__simpan_pembeli">
                        Simpan Data Pembeli
                    </button>
                </div>
            @endif
                ...
                <div class="modal-content border-0 shadow">
                    <div class="modal-body text-center p-4">
                        ...
                        <div class="d-flex justify-content-center gap-2">
                            ...
                            <button type="button" class="btn btn-danger btn-sm px-3 fw-bold" id="btn-confirm-remove-pembeli">Ya, Hapus</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @push('addScript')
            <script>
                $(document).ready(function() {
                    $(".btn__simpan_pembeli").on("click", function() {
                    ...
                    let i_pembeli = {{ $lastIndex }};
                    let pembeliIndexToRemove = null;

                    const pembeliTemplate = (i) => `
                    <div class="card shadow-sm border border-light-subtle rounded-3 card_${i} mt-4" data-index="${i}" style="display:none;">
                        <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle d-flex justify-content-between align-items-center">
                            <h6 class="mb-0 fw-bold text-dark">
                                <i class="bi bi-people-fill me-2 text-primary"></i>Form Data Pembeli
                            </h6>
                            <button type="button" class="btn btn-sm btn-outline-danger border-0 px-2 py-1" onclick="removePembeli(${i})" title="Hapus Form Ini">
                                <i class="bi bi-trash"></i> Hapus
                            </button>
                        </div>
                        <div class="card-body pt-3">
                    ...
                    $(".add_pembeli").on("click", function() {
                        i_pembeli++;
                        let i = i_pembeli;

                        $(".body__pembeli").append(pembeliTemplate(i));
                        $(`.card_${i}`).fadeIn(300);
                    });

                    window.removePembeli = function(idx) {
                        pembeliIndexToRemove = idx;
                        let myModal = new bootstrap.Modal(document.getElementById('modalDeletePembeli'));
                        myModal.show();
                    };

                    $("#btn-confirm-remove-pembeli").on("click", function() {
                        if (pembeliIndexToRemove !== null) {
                            $(`.body__pembeli .card_${pembeliIndexToRemove}`).fadeOut(300, function() {
                                $(this).remove();
                            });
                            $('#modalDeletePembeli').modal('hide');
                            pembeliIndexToRemove = null;
                        }
                    });
                });
            </script>

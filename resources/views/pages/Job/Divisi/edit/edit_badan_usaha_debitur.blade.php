...
@section('title', 'Edit Debitur ' . $jobDivisi->kode)

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-12">
        <!-- FORM 1: FORM UTAMA UPDATE BADAN USAHA DEBITUR -->
        <form
            action="{{ route('job.divisi-data-pendukung-badan-usaha-debitur.update') }}"
            method="POST"
            enctype="multipart/form-data"
        >
            @csrf
            <input type="hidden" name="id" value="{{ $badanUsahaDebitur->id }}">

            <!-- Card Informasi Badan Usaha Debitur -->
            <div class="card shadow-sm border border-light-subtle mb-4 rounded-3">
                <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle">
                    <h5 class="mb-0 fw-bold text-dark">Informasi Badan Usaha Debitur</h5>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nama_badan_usaha', $badanUsahaDebitur->nama_badan_usaha) }}"
                            >
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nama_perwakilan', $badanUsahaDebitur->nama_perwakilan) }}"
                            >
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nomor_telepon', $badanUsahaDebitur->no_telepon) }}"
                            >
                            @error('nomor_telepon')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('email', $badanUsahaDebitur->email) }}"
                            >
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            <div class="card shadow-sm border border-light-subtle mb-4 rounded-3">
                <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary bg-opacity-10 text-primary border border-primary-subtle px-2 py-1">
                        {{ $badanUsahaDebitur->files->count() }} File Tersimpan
                    </span>
                </div>
                <div class="card-body pt-3">
                    <!-- Daftar File Lama -->
                    @if ($badanUsahaDebitur->files->count() > 0)
                        <div class="row g-2 mb-4">
                            @foreach ($badanUsahaDebitur->files as $f)
                                <div class="col-md-6">
                                    <div class="p-3 border border-light-subtle rounded-3 d-flex align-items-center justify-content-between bg-white shadow-sm transition-hover">
                                        ...
                                        <button type="button" form="delete-file-form-badan-usaha-debitur-{{ $f->id }}"
        ...
        @if ($badanUsahaDebitur->files->count() > 0)
            @foreach ($badanUsahaDebitur->files as $f)
                <form id="delete-file-form-badan-usaha-debitur-{{ $f->id }}"
                    action="{{ route('job.divisi-data-pendukung-badan-usaha-debitur.file.destroy', $f->id) }}" method="POST"
                    class="d-none">
                    @csrf
                    @method('DELETE')
                </form>
...
@push('addScript')
    <script>
        // Jalankan langsung tanpa DOMContentLoaded agar aman saat di-push via Blade stack
        document.querySelectorAll('.confirm-delete-file').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                ...
                // Cek apakah library SweetAlert2 (Swal) ada, jika ada gunakan Swal
                if (typeof Swal !== 'undefined') {
                    ...
                    // Fallback jika Swal tidak terload di layout admin

...
@section('title', 'Edit Badan Usaha Penjual ' . $jobDivisi->kode)
 
@section('content')
<div class="row justify-content-center">
    <div class="col-lg-12">
        <!-- FORM 1: FORM UTAMA UPDATE BADAN USAHA PENJUAL -->
        <form
            action="{{ route('job.divisi-data-pendukung-badan-usaha-penjual.update') }}"
            method="POST"
            enctype="multipart/form-data"
        >
            @csrf
            <input type="hidden" name="id" value="{{ $badanUsahaPenjual->id }}">
 
            <!-- Card Informasi Badan Usaha Penjual -->
            <div class="card shadow-sm border border-light-subtle mb-4 rounded-3">
                <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle">
                    <h5 class="mb-0 fw-bold text-dark">Informasi Badan Usaha Penjual</h5>
                </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nama_badan_usaha', $badanUsahaPenjual->nama_badan_usaha) }}"
                            >
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nama_perwakilan', $badanUsahaPenjual->nama_perwakilan) }}"
                            >
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nomor_telepon', $badanUsahaPenjual->no_telepon) }}"
                            >
                            @error('nomor_telepon')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('email', $badanUsahaPenjual->email) }}"
                            >
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            <div class="card shadow-sm border border-light-subtle mb-4 rounded-3">
                <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary bg-opacity-10 text-primary border border-primary-subtle px-2 py-1">
                        {{ $badanUsahaPenjual->files->count() }} File Tersimpan
                    </span>
                </div>
                <div class="card-body pt-3">
                    <!-- Daftar File Lama -->
                    @if ($badanUsahaPenjual->files->count() > 0)
                            ...
                            @foreach ($badanUsahaPenjual->files as $f)
                                <div class="col-md-6">
                                    <div class="p-3 border border-light-subtle rounded-3 d-flex align-items-center justify-content-between bg-white shadow-sm transition-hover">
                                        ...
                                        <button type="button" form="delete-file-form-penjual-{{ $f->id }}"
        ...
        @foreach ($badanUsahaPenjual->files as $f)
            <form id="delete-file-form-penjual-{{ $f->id }}"
                action="{{ route('job.divisi-data-pendukung-badan-usaha-penjual.file.destroy', $f->id) }}" method="POST"
                class="d-none">
                @csrf
                @method('DELETE')
            </form>

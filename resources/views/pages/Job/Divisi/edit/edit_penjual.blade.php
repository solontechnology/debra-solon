...
@section('title', 'Edit Penjual ' . $jobDivisi->kode)

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-12">
        <!-- FORM 1: FORM UTAMA UPDATE PENJUAL -->
        <form
            action="{{ route('job.divisi-data-pendukung-penjual.update') }}"
            method="POST"
        ...
        >
            ...
            <div class="card shadow-sm border border-light-subtle mb-4 rounded-3">
                <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle">
                    <h5 class="mb-0 fw-bold text-dark">Informasi Penjual</h5>
                </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input
                                ...
                                value="{{ old('nomor_telepon', $penjual->nomor_telepon) }}"
                            >
                            @error('nomor_telepon')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            <div class="card shadow-sm border border-light-subtle mb-4 rounded-3">
                <div class="card-header bg-light bg-opacity-50 py-3 border-bottom border-light-subtle d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-primary bg-opacity-10 text-primary border border-primary-subtle px-2 py-1">
                        {{ $penjual->files->count() }} File Tersimpan
                    </span>
                </div>
                <div class="card-body pt-3">
                    <!-- Daftar File Lama -->
                    @if ($penjual->files->count() > 0)
        ...
        @if ($penjual->files->count() > 0)
            @foreach ($penjual->files as $f)
                <form id="delete-file-form-penjual-{{ $f->id }}"
                    action="{{ route('job.divisi-data-pendukung-penjual.file.destroy', $f->id) }}" method="POST"
                    class="d-none">
                    @csrf
                    @method('DELETE')
                </form>

...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('master-data.bank.update', $bank->id) }}" method="post">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3">
                        ...
                        <select name="parent_id" id="parent_id" class="form-select select2"
                            ...
                            @foreach ($banks as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('parent_id', $bank->parent_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nama_pimpinan_sekarang" id="nama_pimpinan_sekarang" class="form-control"
                            value="{{ old('nama_pimpinan_sekarang', $bank->nama_pimpinan_sekarang) }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nama_pimpinan_selanjutnya" id="nama_pimpinan_selanjutnya"
                            class="form-control"
                            value="{{ old('nama_pimpinan_selanjutnya', $bank->nama_pimpinan_selanjutnya) }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="start_kemitraan" id="start_kemitraan" class="form-control"
                            value="{{ old('start_kemitraan', $bank->start_kemitraan) }}">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="end_kemitraan" id="end_kemitraan" class="form-control"
                            value="{{ old('end_kemitraan', $bank->end_kemitraan) }}">
                    </div>
                </div>

                <div class="mt-3">
                    @include('pages.MasterData.Bank.edit._add_kepala_legal_bank', ['bank' => $bank])
                </div>
                <div class="mt-3">
                    @include('pages.MasterData.Bank.edit._add_legal_bank', ['bank' => $bank])
                </div>
                <div class="mt-3">
                    @include('pages.MasterData.Bank.edit._add_kepala_marketing_bank', ['bank' => $bank])
                </div>
                <div class="mt-3">
                    @include('pages.MasterData.Bank.edit._add_marketing_bank', ['bank' => $bank])
                </div>

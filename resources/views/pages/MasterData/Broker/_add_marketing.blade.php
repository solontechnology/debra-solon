@php
    // Kalau validasi gagal, ambil data lama dari input user
    if (old('marketing')) {
        $old_marketing = old('marketing');
    } 
    // Kalau tidak, ambil dari relasi marketing yang udah ada di database
    elseif (isset($item) && $item->marketing->isNotEmpty()) {
        $old_marketing = $item->marketing->map(function ($m) {
            return [
                'nama' => $m->nama,
    ...
    // Kalau dua-duanya ga ada, isi default 1 baris kosong
    else {
        $old_marketing = [
            [
                'nama' => '',
                        ...
                        <div class="col-md-6">
                            <div class="d-flex gap-3 align-items-end">
                                <div class="w-100">
                                    ...
                                    <input type="number" class="form-control" name="marketing[{{ $idxM }}][no_telepon]" 
                           value="{{ $m['no_telepon'] ?? '' }}" required>
                                </div>
                                <div class="">

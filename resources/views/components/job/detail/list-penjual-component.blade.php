<div class="mb-5">
        @if (count($listPenjual) > 0)
            <!-- Header Section -->
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h6 class="fw-bold text-dark mb-0">
                    <i class="bi bi-person-badge-fill text-primary me-2"></i>Daftar Penjual Terdaftar
                </h6>
            </div>

            <!-- Card Table Modern Penjual -->
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0 text-nowrap">
                        ...
                        <tbody class="border-top-0">
                            @foreach ($listPenjual as $penjual)
                                <tr>
                                    <td class="ps-4 fw-bold text-dark">
                                        {{ $penjual->nama ?? '-' }}
                                    </td>
                                    <td>
                                        <span class="text-muted">{{ $penjual->nomor_telepon ?? '-' }}</span>
                                    </td>
                                    <td>
                                        <span class="text-muted">{{ $penjual->email ?? '-' }}</span>
                                    </td>
                                    <td>
                                         @if ($penjual->files && $penjual->files->count() > 0)
                                        <div class="d-flex flex-column gap-1">
                                            @foreach ($penjual->files as $f)
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="d-inline-flex gap-1">
                                            @can('data-pendukung/penjual/edit')
                                                <a href="{{ route('job.divisi-data-pendukung-penjual.edit', $penjual->id) }}"
                                            ...
                                            @endcan
                                            @can('data-pendukung/penjual/delete')
                                                <form action="{{ route('job.divisi-data-pendukung-penjual.delete', $penjual->id) }}"
                                                    method="POST" class="confirm_delete d-inline"
                                                    data-message="penjual {{ $penjual->nama }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                ...
                <div class="py-3">
                    <div class="bg-primary bg-opacity-10 text-primary rounded-circle d-inline-flex p-3 mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor"
                            ...
                            <path d="M2 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V2zm9 9a1 1 0 0 0-1-1H6a1 1 0 0 0-1 1v1h6v-1zM8 8a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/>
                        </svg>
                    </div>
                    <h6 class="fw-bold text-dark">Belum Ada Data Penjual</h6>
                    <p class="text-muted small mb-0">Silakan gunakan form untuk menambahkan data penjual.</p>
                </div>
            </div>
        @endif
    </div>
...
class LemburController extends Controller
{
    ...
    public function index()
    {
        ...
        return view("pages.hris.lembur.index", [
    ...
    {
        return view("pages.hris.lembur.create");
    }
    ...
    public function store(Request $request)
    {
        ...
        //     "file" => $request->file ? $request->file->store("lembur", "public") : null,
        ...
        $cuti = Lembur::create([
            ...
            "end_date" => $request->end_date,
            "file" => $request->file
                ? $request->file->store("lembur", "public")
                : null,
            "user_id" => Auth::user()->id
        ]);
        ...
        foreach ($superAdmins as $admin) {
            ...
            $this->notifikasiServis->create(
                $admin->id,
                "Pengajuan Lembur",
                Auth::user()->name . " mengajukan lembur",
                route('hris.lembur.index')
            );
        }

        return redirect()->route("hris.lembur.index")->with("success", "Berhasil menambahkan cuti baru");
    }
    ...
    {
        $cuti = Lembur::findOrFail($id);
        $cuti->status = $request->status;
        $cuti->approved_by = Auth::user()->id;
        $cuti->save();

        return redirect()->route("hris.lembur.index")->with("success", "Berhasil $request->status cuti");
    }
